...

{
  ...
  var updateRelatedSelectsOptions = function updateRelatedSelectsOptions(currentSelect, win, objId, newRepr, newId) {
    var skipIds = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : [];
    ...
    var modelName = path.split('/')[path.split('/').length - (objId ? 4 : 3)]; // Select elements with a specific model reference and context of "available-source".

    var selectsRelated = document.querySelectorAll("[data-model-ref=\"".concat(modelName, "\"] [data-context=\"available-source\"]"));
    selectsRelated.forEach(function (select) {
      ...
      if (!option) {
        ...
        select.options.add(option); // Update SelectBox cache for related fields.
  ...
  window.relatedWindows = relatedWindows; // Kept for backward compatibility
