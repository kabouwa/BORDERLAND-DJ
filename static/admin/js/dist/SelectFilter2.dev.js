...

{
  window.SelectFilter = {
    init: function init(field_id, field_name, is_stacked) {
      ...
      var selector_div = quickElement('div', from_box.parentNode); // Make sure the selector div is at the beginning so that the
      ...
      var choose_all = quickElement('button', selector_available, interpolate(gettext('Choose all %s'), [field_name]), 'id', field_id + '_add_all', 'class', 'selector-chooseall', 'type', 'button'); // <ul class="selector-chooser">

      var selector_chooser = quickElement('ul', selector_div);
      selector_chooser.className = 'selector-chooser';
      var add_button = quickElement('button', quickElement('li', selector_chooser), interpolate(gettext('Choose selected %s'), [field_name]), 'id', field_id + '_add', 'class', 'selector-add', 'type', 'button');
      var remove_button = quickElement('button', quickElement('li', selector_chooser), interpolate(gettext('Remove selected %s'), [field_name]), 'id', field_id + '_remove', 'class', 'selector-remove', 'type', 'button'); // <div class="selector-chosen">
      ...
      var clear_all = quickElement('button', selector_chosen, interpolate(gettext('Remove all %s'), [field_name]), 'id', field_id + '_remove_all', 'class', 'selector-clearall', 'type', 'button');
    ...
    refresh_icons: function refresh_icons(field_id) {
      ...
      var to = document.getElementById(field_id + '_to'); // Disabled if no items are selected.

      document.getElementById(field_id + '_add').disabled = !SelectFilter.any_selected(from);
      document.getElementById(field_id + '_remove').disabled = !SelectFilter.any_selected(to); // Disabled if the corresponding box is empty.
    ...
    filter_key_press: function filter_key_press(event, field_id, source, target) {
      var source_box = document.getElementById(field_id + source); // don't submit form if user pressed Enter
    ...
    filter_key_down: function filter_key_down(event, field_id, source, target) {
      var source_box = document.getElementById(field_id + source); // right key (39) or left key (37)

      var direction = source === '_from' ? 39 : 37; // right arrow -- move across
